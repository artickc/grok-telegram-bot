...
describe("ext-methods plan exit", () => {
  it("detects exit_plan method names including _x.ai prefix", () => {
    assert.equal(isExitPlanMethod("_x.ai/exit_plan_mode"), true);
  ...
  it("handles _x.ai/exit_plan_mode via reverse handler", async () => {
    const result = await handleAgentReverseRequest("_x.ai/exit_plan_mode", { plan_content: "p" }, () => ({
      outcome: "approved",
      feedback: "go",
    }));
  ...
  it("skips ask_user_question so headless turns do not hang", async () => {
