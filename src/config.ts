...
export interface AppConfig {
  /** Primary bot token (BOT_TOKEN_1, TELEGRAM_BOT_TOKEN, or the first listed token). */
  token: string;
...
export function loadConfig(): AppConfig {
  ...
  const cfg: AppConfig = {
    ...
    sttApiKey: process.env.STT_API_KEY?.trim() || process.env.XAI_API_KEY?.trim() || undefined,
    sttModel: process.env.STT_MODEL?.trim() || "grok-stt",
